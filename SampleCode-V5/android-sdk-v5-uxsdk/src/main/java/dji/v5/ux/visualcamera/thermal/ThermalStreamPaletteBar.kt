...
class ThermalStreamPaletteBar @JvmOverloads constructor(
    ...
    private fun updateTemperature(it: ThermalAreaMetersureTemperature) {
        ...
        binding.leftTv.text = convertTemperature(it.minAreaTemperature).toInt().toString()
        binding.rightTv.text = convertTemperature(it.maxAreaTemperature).toInt().toString()
    }
    ...
    private fun updateCurrentPaletteModelPosition(type: CameraThermalPalette) {
        ...
        model?.let {
            binding.ivPalettePreview.setImageResource(it.colorBar)
        }
    }
